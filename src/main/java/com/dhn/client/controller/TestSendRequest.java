...
@Component
@Slf4j
public class TestSendRequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        param.setTest_use(appContext.getEnvironment().getProperty("dhnclient.test_flag","N"));
        param.setDatabase(appContext.getEnvironment().getProperty("dhnclient.database"));
        param.setSequence(appContext.getEnvironment().getProperty("dhnclient.at_seq"));
        param.setMsg_type("TT");

        dhnServer = appContext.getEnvironment().getProperty("dhnclient.server");
        userid = appContext.getEnvironment().getProperty("dhnclient.userid");

        if (param.getTest_use() != null && param.getTest_use().equalsIgnoreCase("Y")) {
            isStart = true;
            log.info("TEST 초기화 완료");
        } else {
    ...
    public void SendProcess() {
        if(isStart && !isProc) {
            ...
            if(activeThreads < 4){
                ...
                String group_no = "TEST" + now.format(formatter);
                ...
                if(!group_no.equals(preGroupNo)) {
                    try{
                        ...
                        log.error("TEST 메세지 전송 오류 : " + e.toString());
                    }
        ...
        try{
            ...
            try {
                ...
                if (response.getStatusCode() == HttpStatus.OK) { // 데이터 정상적으로 전달
                    ...
                    log.info("TEST 메세지 전송 완료 : " + response.getStatusCode() + " / " + group_no + " / " + _list.size() + " 건");
                }else { // API 전송 실패시
                    log.info("({}) TEST 메세지 전송오류 : {}",res.get("userid"), res.get("message"));
